...
//        logContext.getLogger("us.codecraft.webmagic.downloader.HttpClientDownloader").setLevel(Level.WARN);
    ...
    public static void setLoggerLevel(Map<String, Level> levelMap) {
        if (levelMap==null || levelMap.size()==0){
            return;
        }
        LoggerContext logContext = (LoggerContext) LoggerFactory.getILoggerFactory();
        for (String classPackage:levelMap.keySet()) {
            logContext.getLogger(classPackage).setLevel(levelMap.get(classPackage));
        }
    }
}
